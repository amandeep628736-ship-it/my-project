...
function Login() {
  ...
  function loginHandle(e)
    {
        if(user.length<3 || password.length<8 || !isChecked)
        {
            alert("type correct values and check the box")
        }
        else
        {
            alert("all good :)")
    ...
    return (
        <div>
           ...
           <input type="text" placeholder="Enter User Id" onChange={userHandler} />{userErr?<span>User Not Valid</span>:""}
            ...
            <br /> <br />
            <input type="checkbox" onChange={(e) => setIsChecked(e.target.checked)} /> Agree to terms
            <br /> <br />
